"""OpenAI-compatible provider bundle (``OpenAIProviderBundle``).

Extracted from ``paper_chaser_mcp.agentic.provider_openai`` during
Phase 8c of the modularization refactor. The shim at
``paper_chaser_mcp.agentic.provider_openai`` re-exports the public
bundle classes so existing imports keep working unchanged.
"""
...
__all__ = ["OpenAIProviderBundle"]


def _execute_provider_call_sync(**kwargs: Any) -> Any:
    from .. import execute_provider_call_sync

    return execute_provider_call_sync(**kwargs)


async def _execute_provider_call(**kwargs: Any) -> Any:
    from .. import execute_provider_call

    return await execute_provider_call(**kwargs)
