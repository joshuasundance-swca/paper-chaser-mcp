...
from paper_chaser_mcp.dispatch.guided.inspect_source import (
...
def test__guided_extract_question_pulls_first_nonempty_field() -> None:
    assert _guided_extract_question({"question": "q"}) == "q"
    assert _guided_extract_question({"prompt": "p"}) == "p"
    assert _guided_extract_question({"query": "x"}) == "x"
    assert _guided_extract_question({}) is None


def test__guided_compact_source_candidate_returns_dict() -> None:
    out = _guided_compact_source_candidate({"sourceId": "s1", "title": "t", "bogus": "ignore"})
    assert out.get("sourceId") == "s1"


def test__guided_source_resolution_payload_returns_dict() -> None:
    out = _guided_source_resolution_payload(
