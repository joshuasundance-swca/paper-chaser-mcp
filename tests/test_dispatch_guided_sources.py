...
from paper_chaser_mcp.dispatch.guided.sources import (
...
def test__guided_source_id_falls_through_keys_then_title_then_fallback() -> None:
    assert _guided_source_id({"title": "Weather Report"}, fallback_prefix="src", index=3) == "Weather Report"
    assert _guided_source_id({}, fallback_prefix="paper", index=7) == "paper-7"


def test__guided_extract_source_id_picks_first_non_none() -> None:
    assert _guided_extract_source_id({"sourceId": "a"}) == "a"
    assert _guided_extract_source_id({"evidence_id": "b"}) == "b"
    assert _guided_extract_source_id({"leadId": "c"}) == "c"
    assert _guided_extract_source_id({}) is None
...
def test__guided_sources_from_fr_documents_skips_missing_title() -> None:
    ...
    out = _guided_sources_from_fr_documents("endangered species", docs)
    assert len(out) == 1
...
def test__guided_sources_from_fr_documents_handles_rule_type() -> None:
    ...
    out = _guided_sources_from_fr_documents("x", docs)
...
def test__guided_dedupe_source_records_collapses_by_identity() -> None:
    ...
    out = _guided_dedupe_source_records(records)
    assert len(out) == 2
...
def test__guided_source_matches_reference_title_and_url() -> None:
    ...
    assert _guided_source_matches_reference(candidate, "Climate Paper")
    assert _guided_source_matches_reference(candidate, "https://example.com/abc")
    assert not _guided_source_matches_reference(candidate, "")
    assert not _guided_source_matches_reference(candidate, "something else")


def test__guided_source_records_share_surface_title_overlap() -> None:
    assert _guided_source_records_share_surface(
        {"title": "Same Title"},
        {"title": "SAME TITLE"},
    )
    assert not _guided_source_records_share_surface(
        {"title": "A"},
        {"title": "B"},
    )
...
def test__guided_source_identity_returns_lowercase_title_tuple() -> None:
    assert _guided_source_identity({"sourceId": "s", "canonicalUrl": "U", "title": "Foo"}) == (
        "s",
        "U",
        "foo",
    )
...
def test__guided_merge_source_records_prefers_primary_then_fills() -> None:
    ...
    merged = _guided_merge_source_records(primary, secondary)
...
def test__guided_merge_source_record_sets_collapses_duplicates() -> None:
    ...
    out = _guided_merge_source_record_sets(a, b)
    assert len(out) == 1
    assert out[0]["isPrimarySource"] is True


def test__guided_source_coverage_summary_empty_returns_none() -> None:
    assert _guided_source_coverage_summary(sources=[], leads=[], base_coverage=None) is None


def test__guided_source_coverage_summary_counts_by_access_status() -> None:
    result = _guided_source_coverage_summary(
