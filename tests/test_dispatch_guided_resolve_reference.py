...
def test__guided_note_repair_noop_when_equal() -> None:
    ...
    rr._guided_note_repair(
        repairs, field="title", original="x", normalized="x", reason="r"
    )
    assert repairs == []
...
def test__guided_note_repair_records_change() -> None:
    repairs: list[dict[str, str]] = []
    rr._guided_note_repair(
        repairs, field="title", original="A ", normalized="A", reason="trim"
    )
    assert repairs == [
        {"field": "title", "from": "A ", "to": "A", "reason": "trim"}
    ]


def test__guided_underspecified_reference_returns_none_for_clear_ident() -> None:
    assert rr._guided_underspecified_reference_clarification(
        query="10.1234/abcd", focus=None
    ) is None
