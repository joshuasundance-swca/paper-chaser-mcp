...
from paper_chaser_mcp.dispatch.guided.resolve_reference import (
    _guided_note_repair,
...
def test__guided_note_repair_noop_when_equal() -> None:
    ...
    _guided_note_repair(repairs, field="title", original="x", normalized="x", reason="r")
    assert repairs == []


def test__guided_note_repair_records_change() -> None:
    repairs: list[dict[str, str]] = []
    _guided_note_repair(repairs, field="title", original="A ", normalized="A", reason="trim")
...
def test__guided_underspecified_reference_returns_none_for_clear_ident() -> None:
    assert _guided_underspecified_reference_clarification(query="10.1234/abcd", focus=None) is None


def test__guided_underspecified_reference_returns_none_for_empty_query() -> None:
    assert _guided_underspecified_reference_clarification(query="", focus=None) is None
