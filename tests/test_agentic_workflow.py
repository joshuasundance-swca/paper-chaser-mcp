...
def test_agentic_workflow_source_and_lockfile_are_checked_in() -> None:
    ...
    workflow_source = workflow_md.read_text()
    workflow_compiled = workflow_lock.read_text()

    assert "mcp-servers:" in workflow_source
    assert "scholar-search:" in workflow_source
    assert 'search_papers(query="graph neural networks", limit=5)' in workflow_source
    assert "search_papers_bulk" in workflow_source
    assert "search_papers_match" in workflow_source
    assert "create at most one GitHub" in workflow_source

    assert "automatically generated by gh-aw" in workflow_compiled
    assert 'name: "Test Scholar Search MCP"' in workflow_compiled
    assert "github/gh-aw/actions/setup@" in workflow_compiled
