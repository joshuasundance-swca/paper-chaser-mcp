...
def test_agentic_workflow_source_and_lockfile_are_checked_in() -> None:
    ...
    assert "{{#runtime-import .github/workflows/test-scholar-search.md}}" in (workflow_lock_content)
...
def test_agentic_workflow_documentation_stays_in_sync() -> None:
    ...
    assert "pre-commit run --files .github/workflows/test-scholar-search.lock.yml || true" in validate_workflow
    assert "pre-commit run --files .github/workflows/test-scholar-search.lock.yml" in (validate_workflow)
    assert "git diff --exit-code -- .github/workflows/test-scholar-search.lock.yml" in (validate_workflow)
...
def test_verifier_workflow_is_manual_only_for_public_repo_safety() -> None:
    ...
    repo_root = Path(__file__).resolve().parent.parent
    workflow_md = (repo_root / ".github" / "workflows" / "test-scholar-search.md").read_text()

    assert "workflow_dispatch:" in workflow_md
    assert "\n  push:\n" not in workflow_md
    assert "manual-only by design" in workflow_md, (
        "test-scholar-search.md should keep the secret-bearing verifier manual-only in this public repository."
    )
