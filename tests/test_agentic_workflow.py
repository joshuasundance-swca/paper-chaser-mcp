...
def test_agentic_workflow_source_and_lockfile_are_checked_in() -> None:
    ...
    workflow_md = repo_root / ".github" / "workflows" / "test-paper-chaser.md"
    workflow_lock = repo_root / ".github" / "workflows" / "test-paper-chaser.lock.yml"
    ...
    assert 'name: "Test Paper Chaser MCP"' in workflow_lock_content
    assert (
        "github/gh-aw-actions/setup@" in workflow_lock_content or "github/gh-aw/actions/setup@" in workflow_lock_content
    )
    assert "{{#runtime-import .github/workflows/test-paper-chaser.md}}" in (workflow_lock_content)
...
def test_agentic_workflow_documentation_stays_in_sync() -> None:
    ...
    compile_command = "gh aw compile test-paper-chaser --dir .github/workflows"

    assert ".github/workflows/test-paper-chaser.md" in readme
    ...
    assert "pre-commit run --files .github/workflows/test-paper-chaser.lock.yml || true" in validate_workflow
    assert "pre-commit run --files .github/workflows/test-paper-chaser.lock.yml" in (validate_workflow)
    assert "git diff --exit-code -- .github/workflows/test-paper-chaser.lock.yml" in (validate_workflow)


def test_agentic_assign_workflow_catches_workflow_created_issues() -> None:
    repo_root = Path(__file__).resolve().parent.parent
    assign_workflow = (repo_root / ".github" / "workflows" / "agentic-assign.yml").read_text()

    assert "issues:" in assign_workflow
    assert "workflow_run:" in assign_workflow
    assert 'workflows: ["Test Paper Chaser MCP"]' in assign_workflow
    ...
    expected_token_fallback = "secrets.GH_AW_GITHUB_TOKEN || secrets.COPILOT_GITHUB_TOKEN || github.token"
...
def test_verifier_workflow_is_manual_only_for_public_repo_safety() -> None:
    ...
    repo_root = Path(__file__).resolve().parent.parent
    workflow_md = (repo_root / ".github" / "workflows" / "test-paper-chaser.md").read_text()

    assert "workflow_dispatch:" in workflow_md
    assert "\n  push:\n" not in workflow_md
    assert "manual-only by design" in workflow_md, (
        "test-paper-chaser.md should keep the secret-bearing verifier manual-only in this public repository."
    )
